...
export default class SdoSalesGptAssistant extends XdoToolTrackingEventHandlerBase {
	...
	// Data
	responses;
	einsteinGreeting = this.configureMessage("", "", "", "Hi! I’m Einstein, an AI assistant. I can do things like search for information, summarize records, and draft and revise emails. What can I help you with?", "greeting", "slds-size_1-of-1 result0", "");
	@track messages = [];
	description;
	@track emailBody;
	inputVariables;
	...
	connectedCallback() {
		...
		if (this.hiddenDefault == true) {
			...
			if (this.headerDarkMode == true) {
				this.einsteinMenuIconClass = 'einsteinMenuIconInverse';
			} else {
				this.einsteinMenuIconClass = 'einsteinMenuIcon';
			}
		} else {
			this.welcomeLoading = true;
	...
	toggleOverlay(event) {
		if (this.overlayHidden == false) {
			...
			if (this.headerDarkMode == true) {
				this.einsteinMenuIconClass = 'einsteinMenuIconActiveInverse';
			} else {
				this.einsteinMenuIconClass = 'einsteinMenuIconActive';
			}
		}
	}
